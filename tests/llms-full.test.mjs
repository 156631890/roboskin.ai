...
test('curated llms file and homepage head expose machine-readable discovery', () => {
  ...
  assert.match(llmsSource, /177 source-reviewed knowledge entities/);
  assert.match(llmsSource, /28 papers, 1 documentation record, 20 datasets, 11 benchmarks, 13 sensors, 19 robot AI models, 63 verified organizations, and 22 normalized robot-platform records/);
  assert.match(llmsSource, /232 deduplicated primary and official source records/);
  assert.match(llmsSource, /41 model-organization relations/);
  assert.match(llmsSource, /42 model-robot relations/);
  assert.match(llmsSource, /95 research-provenance relations, and 28 semantic entity relations/);
