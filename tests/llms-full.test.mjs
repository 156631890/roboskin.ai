...
test('curated llms file and homepage head expose machine-readable discovery', () => {
  ...
  assert.match(llmsSource, /141 source-reviewed knowledge entities/);
  assert.match(llmsSource, /27 papers, 1 documentation record, 19 datasets, 11 benchmarks, 13 sensors, 13 robot AI models, 41 verified organizations, and 16 normalized robot-platform records/);
  assert.match(llmsSource, /194 deduplicated primary and official source records/);
  assert.match(llmsSource, /22 model-organization relations/);
  assert.match(llmsSource, /31 model-robot relations/);
  assert.match(llmsSource, /74 research-provenance relations, and 25 semantic entity relations/);
  assert.match(llmsSource, /9 narrowly bounded model-to-dataset training relations/);
