...
test('curated llms file and homepage head expose machine-readable discovery', () => {
  ...
  assert.match(llmsSource, /191 source-reviewed knowledge entities/);
  assert.match(llmsSource, /29 papers, 1 documentation record, 22 datasets, 12 benchmarks, 14 sensors, 20 robot AI models, 69 verified organizations, and 24 normalized robot-platform records/);
  assert.match(llmsSource, /245 deduplicated primary and official source records/);
  ...
  assert.match(llmsSource, /13 official-source-bounded sensor-or-robot manufacturer\/provider relations/);
  assert.match(llmsSource, /11 narrowly bounded model-to-dataset training relations/);
  assert.match(llmsSource, /datasets#dataset-egotouch/);
  assert.match(llmsSource, /benchmarks#benchmark-touchworld-real-robot/);
  ...
  assert.match(llmsSource, /research\/vision-based-tactile-intelligence-robotics-survey-2026/);
  assert.match(llmsSource, /research\/univtac-platform-encoder-benchmark-2026/);
