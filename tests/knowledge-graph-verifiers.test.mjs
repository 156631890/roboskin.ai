...
test('export and production verifiers enforce knowledge graph v1.3 provenance', async () => {
  ...
  for (const verifier of verifiers) {
    assert.match(verifier, /researchRelations = \['sourceAffiliation', 'partOf', 'usesSensor', 'usesRobot'\]/);
    ...
    assert.match(verifier, /\+ graph\.counts\?*\.researchProvenanceEdges/);
