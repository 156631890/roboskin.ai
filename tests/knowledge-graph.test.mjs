...
test('knowledge graph JSON is a protected deterministic static output outside the HTML sitemap', async () => {
  ...
  assert.equal(contract.counts.sourceDocuments, 245);
  assert.equal(contract.counts.edges, 569);
  assert.equal(contract.counts.supportedByEdges, 324);
  ...
  assert.equal(contract.counts.trainedAcrossEdges, 14);
  ...
  assert.equal(contract.counts.researchRelationEdges, 145);
  assert.equal(contract.counts.researchProvenanceEdges, 110);
  assert.equal(contract.counts.researchSemanticEdges, 35);
  assert.equal(contract.counts.sourceAffiliationEdges, 77);
  assert.equal(contract.counts.manufacturedByEdges, 13);
  assert.equal(contract.counts.introducesEdges, 16);
  assert.equal(contract.counts.describesDatasetEdges, 4);
  assert.equal(contract.counts.usesDatasetEdges, 0);
  assert.equal(contract.counts.trainedOnEdges, 11);
  assert.equal(contract.counts.evaluatedByEdges, 4);
