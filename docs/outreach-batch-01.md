# RoboSkin.ai outreach batch 01

Prepared: 2026-08-16
Owner: RoboSkin.ai Editorial Team
Reply address: contact@roboskin.ai
Status: Five outreach messages sent individually on 2026-08-16 from `messigoat147@gmail.com`; Gmail Sent Mail verification completed.

## Campaign asset

- New research news: https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026
- Research index: https://roboskin.ai/research-index
- Physical AI definition: https://roboskin.ai/physics-ai
- Humanoid robot skin guide: https://roboskin.ai/applications/humanoid-robot-skin

The campaign offers a useful editorial source. It does not ask for a paid link, reciprocal link, or unqualified endorsement.

## 01 — Robohub

- Recipient: editors@robohub.org
- Verified route: https://robohub.org/contribute/
- Fit: Robohub explicitly accepts robotics news tips and article pitches of 100 words or less.
- Status: Sent 2026-08-16 from `messigoat147@gmail.com`
- Subject: News tip: hybrid robot skin pairs EIT maps with pneumatic force sensing

Body:

Hello Robohub Editors,

RoboSkin.ai has published a source-backed brief on a 2026 preprint that combines electrical impedance tomography with pneumatic sensing in a 280 × 280 mm humanoid chest skin. The authors report 100 Hz EIT reconstruction and a reduction in sensitivity non-uniformity from 0.31 to 0.14 coefficient of variation. Our brief separates the reported result from deployment limits, including pad-overlap sensitivity and the absence of closed-loop safety validation.

Brief: https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026

If useful, we can adapt this into an original, non-promotional guest explainer on why whole-body robot skin is becoming a sensor-fusion problem.

RoboSkin.ai Editorial Team
contact@roboskin.ai

## 02 — RoboticsTomorrow

- Recipient: news@roboticstomorrow.com
- Verified route: https://www.roboticstomorrow.com/contact.php
- Fit: The site lists this address for editorial submissions and publishes robotics news.
- Status: Sent 2026-08-16 from `messigoat147@gmail.com`
- Response: Brandon Hetherington confirmed on 2026-08-16 that RoboticsTomorrow would prepare the article and send the live link. Await publication; do not count a backlink until the resulting page is crawlable and links to RoboSkin.ai.
- Subject: Robotics research brief: 3D-printed hybrid skin for humanoid force maps

Body:

Hello RoboticsTomorrow Editorial Team,

I am sharing a concise research brief on a new large-area tactile sensing approach for humanoid robots. The cited preprint combines 32-electrode EIT localization with four pneumatic pads in a digitally manufactured 280 × 280 mm chest skin. It reports real-time 100 Hz reconstruction and lower location-dependent sensitivity variation than its EIT-only baseline.

RoboSkin.ai brief: https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026
Original paper: https://arxiv.org/abs/2605.28468

The brief includes the engineering limitations and is available as a source tip for your robotics research coverage. We can also provide a clean summary or original contributed article if that better fits your editorial process.

Best,
RoboSkin.ai Editorial Team
contact@roboskin.ai

## 03 — The Robot Report

- Recipient: moitzman@wtwhmedia.com
- Verified route: https://www.therobotreport.com/author/moitzman/
- Fit: Mike Oitzman is listed as a senior editor and covers robotics systems and industry engineering.
- Status: Sent 2026-08-16 from `messigoat147@gmail.com`
- Subject: Source tip for humanoid robotics: hybrid whole-body tactile skin

Body:

Hello Mike,

For your humanoid and robotics-systems coverage, this 2026 preprint may be worth tracking: an EIT–pneumatic hybrid skin fabricated by 3D printing and spray coating, then mounted on a humanoid chest. The authors use EIT for spatial cues and pneumatic pads for net-force calibration, reporting a sensitivity coefficient of variation change from 0.31 to 0.14 in their indentation protocol.

Our evidence-bounded summary is here:
https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026

We also maintain a machine-readable tactile research index if a comparison source is useful:
https://roboskin.ai/research-index

No affiliation is implied; this is an editorial source tip.

Best,
RoboSkin.ai Editorial Team
contact@roboskin.ai

## 04 — Robotics & Automation News

- Recipient: markallinson@roboticsandautomationnews.com
- Verified route: https://roboticsandautomationnews.com/about/contact/
- Fit: Mark Allinson is listed as an editor; the outlet covers humanoids, automation, and robotics engineering.
- Status: Published 2026-08-19; verified editorial link to `https://roboskin.ai/`
- Result: https://roboticsandautomationnews.com/2026/08/19/researchers-combine-eit-and-pneumatic-sensing-for-humanoid-robot-skin/104274/
- Link audit: Body copy links the natural anchor `RoboSkin` to the homepage with `rel="noopener"`; no `nofollow`, `sponsored`, or `ugc` attribute was present when checked on 2026-08-19.
- Subject: Editorial source: why Physical AI needs force maps, not only contact flags

Body:

Hello Mark,

RoboSkin.ai has prepared a research brief that may fit your humanoid and Physical AI coverage. A 2026 preprint combines electrical impedance tomography with pneumatic force sensing in a large-area robot skin. The useful systems angle is the division of labor: EIT preserves contact location while the pneumatic chamber anchors force magnitude.

Brief: https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026
Physical AI context: https://roboskin.ai/physics-ai

The article includes the reported 280 × 280 mm area, 100 Hz reconstruction, 32 electrodes, four pads, and the paper's stated overlap-region limitation. We can prepare an original non-promotional contribution on the complete path from contact surface to robot control if useful.

Best,
RoboSkin.ai Editorial Team
contact@roboskin.ai

## 05 — ManufacturingTomorrow

- Recipient: articles@bthmgmt.com
- Verified route: https://www.manufacturingtomorrow.com/associates.php
- Fit: The outlet explicitly invites articles, case studies, and interviews by email.
- Status: Sent 2026-08-16 from `messigoat147@gmail.com`
- Subject: Contributed article proposal: the engineering stack behind tactile Physical AI

Body:

Hello ManufacturingTomorrow Editorial Team,

I would like to propose an original, vendor-neutral article: “The engineering stack behind tactile Physical AI.” It would explain why contact-rich automation needs more than a sensor specification, covering surface design, pressure/shear/slip signals, calibration, timestamps, robot middleware, controller latency, and task-level evidence.

Supporting research brief:
https://roboskin.ai/news/eit-pneumatic-hybrid-robot-skin-force-map-2026

Supporting public data index:
https://roboskin.ai/research-index

The article would be educational, source-linked, and written specifically for manufacturing engineers rather than republished from our site. Please let me know whether a 1,200–1,600 word draft or a short Q&A format better fits your calendar.

Best,
RoboSkin.ai Editorial Team
contact@roboskin.ai

## Execution record

1. Five messages were sent individually from `messigoat147@gmail.com` on 2026-08-16 at approximately 01:12 Asia/Shanghai.
2. Gmail Sent Mail was checked after sending and displayed all five exact subjects.
3. No recipients were combined and no bulk-send feature was used.
4. Record each response and earned link in this file.
5. Follow up once between 2026-08-25 and 2026-08-28 only when the target has not replied.
6. Robotics & Automation News published the pitched story on 2026-08-19. Do not send the scheduled follow-up; a single thank-you and optional deep-link correction request may be sent instead.
7. RoboticsTomorrow confirmed that it is preparing the submitted article. Wait for the promised publication link rather than sending a follow-up.
