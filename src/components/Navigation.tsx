          ...
          <div className="hidden md:flex items-center space-x-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={"px-4 py-2 rounded-lg text-sm font-semibold transition-all " + (pathname === link.href ? "text-blue-600 bg-blue-50" : "text-gray-700 hover:text-blue-600 hover:bg-gray-50")}
              >
                {link.label}
              </Link>
            ))}
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-200">
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMobileMenuOpen(false)} className={"block px-4 py-3 rounded-lg text-sm font-semibold " + (pathname === link.href ? "bg-blue-50 text-blue-600" : "text-gray-700 hover:bg-gray-50")}>
                {link.label}
              </Link>
            ))}
          </div>
