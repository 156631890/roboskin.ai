...
export const robotAiModelEntries: RobotAiModelEntry[] = [
  {
    ...
    trainingDataSummary: "The v1 paper pretrains the encoder on 205,826 simulation samples from 14 geometric primitives with marked/marker-free tactile images, depth, projected markers and 7D relative pose. The September release lists 638 contact HDF5 files; their frame-level equivalence to that corpus is unverified. These are separate from versioned task episodes, the paper's 400 policy-training trajectories and 450 physical demonstrations.",
    ...
    availability: "Code, a hosted encoder checkpoint and separate Isaac Sim 4.5/5.1 task-file directories are public. The September 19 README associates released policy checkpoints with 4.5; it does not establish 5.1 policy compatibility. Collection and evaluation still list simulated GelSight Mini as supported, with GF225 and Xense WS planned. See the linked review for pinned files and audit limits.",
  ...
  },
  {
    id: 't-rex',
    name: 'T-Rex',
    organization: 'UC Berkeley, NVIDIA, Stanford University, Panasonic, Sapienza University of Rome, and ItalAI',
    creatorOrganizations: ['UC Berkeley', 'NVIDIA', 'Stanford University', 'Panasonic', 'Sapienza University of Rome', 'ItalAI'],
    releaseDate: '2026-06-15',
    category: 'VLA',
    inputModalities: [
      'Natural-language instruction',
      'Head and bilateral wrist RGB video',
      'Robot state',
      'Ten fingertip tactile deformation streams and 6D wrench estimates',
    ],
    outputType: 'Bimanual robot-action chunks with approximately 5 Hz slow action generation and 20 Hz tactile refinement',
    embodiments: ['Fixed-base bimanual Dexmate Vega-1 with two 22-DoF Sharpa Wave hands'],
    trainingDataSummary: 'The authors report three stages: large-scale tactile-free egocentric pretraining, tactile-reactive midtraining on the full 100-hour T-Rex collection, and task-specific post-training. The current public dataset card specifies 5,464 episodes and 5,473,459 frames at 30 FPS, approximately 50 hours, rather than the complete 100-hour training corpus.',
    realRobotEvaluation: 'The official project reports a 65% macro-average for T-Rex versus 35% for EgoScale across 12 contact-rich tasks, with 16 randomized rollouts per task on one fixed-base bimanual setup.',
    availability: 'Paper, project page, MIT-licensed repository, pretrained and midtrained checkpoints, post-training and inference code, full-pipeline branch, and an approximately 50-hour LeRobot v3.0 dataset subset are public.',
  ...
  },
  {
    ...
    creatorOrganizations: ['Beijing Institute of Technology'],
    releaseDate: '2026-08-16',
    category: 'VLA',
    inputModalities: [
      'Natural-language instruction',
      'Primary scene or head RGB image',
      'Wrist RGB image',
      'Robot proprioception',
      'Marker-derived contact descriptor',
      'Bilateral tactile-image summary',
    ],
    outputType: 'Frozen OpenVLA-OFT action chunk plus an optional bounded 7D end-effector residual selected and scaled from current contact evidence',
    embodiments: [
    ...
    trainingDataSummary: 'The frozen OpenVLA-OFT reference is fine-tuned on 50 UniVTAC trajectories per task across seven tasks. ViTaR additionally uses 20 restored decision groups per task and short local branches to supervise effect ranking and residual modulation; trajectory, decision-group, branch, and preference-pair counts are different units and are not combined here as one demonstration total.',
    realRobotEvaluation: 'The authors report binary end-to-end success on three physical tasks with 20 trials per method-task pair: 40% on Insert Hole, 65% on Lift Bottle, and 40% on Wiping the Board, or 48.3% across those tasks. A separate seven-task UniVTAC protocol uses 100 episodes per method-task pair and reports a 61.3% average.',
    availability: 'Paper, official project page, video, figures, and project-page source are public. The official page marks code as “Coming soon”; no runnable ViTaR implementation, model weights, EGM or RAM checkpoints, exact branch corpus, or ViTaR-specific dataset download was verified.',
  ...
  },
  {
    id: 'retouch',
    name: 'ReTouch',
    organization: 'University of Science and Technology of China, iFLYTEK, and The Chinese University of Hong Kong',
    creatorOrganizations: ['University of Science and Technology of China', 'iFLYTEK', 'The Chinese University of Hong Kong'],
    releaseDate: '2026-08-03',
    category: 'VLA',
    inputModalities: [
      'Natural-language task instruction',
      'One wrist-mounted RGB view',
      'Two fixed external RGB views',
      '18-dimensional robot proprioception',
      'Current frame plus the preceding nine frames from five fingers with 120 three-axis force taxels per finger (a ten-frame input window)',
    ],
    outputType: 'Sixteen-step chunks of 18-dimensional absolute joint-position commands, with the remaining suffix regenerated from updated tactile observations during execution',
    embodiments: ['UR7e arm with an XHand five-finger tactile dexterous hand, one wrist RGB camera, and two fixed external RGB cameras'],
    trainingDataSummary: 'XHT-Dataset contains 900 retained successful demonstrations across seven real-world tasks. The paper holds out 100 trajectories from tactile-encoder and policy training and uses the remaining 800 as a common training pool; it does not disclose the per-task distribution or publish the split manifest or dataset files.',
    realRobotEvaluation: 'The standard protocol uses 20 real-robot rollouts per method on each of seven tasks and reports an 83.6% macro-average normalized task score. Button Press is binary, while the other six tasks award weighted partial credit. Four separate challenging settings also use 20 rollouts per method-setting and report a 73.1% macro-average for ReTouch.',
    availability: 'The current arXiv v2 paper is public. No paper-linked official project page, source-code repository, model weights, checkpoints, XHT-Dataset download, or exact split manifest was verified on 2026-08-22.',
  ...
  {
    id: 'tau-touch-augmented-vla',
    name: 'τ (Touch-Augmented VLA)',
    organization: 'Beijing Jiaotong University and Beijing Institute for General Artificial Intelligence',
    creatorOrganizations: ['Beijing Jiaotong University', 'Beijing Institute for General Artificial Intelligence'],
    releaseDate: '2026-07-27',
    category: 'VLA',
    inputModalities: ['Natural-language instruction', 'Multi-view RGB observation', 'Robot proprioception', 'Bilateral vision-based tactile deformation'],
    outputType: 'Future robot-action chunks generated by a π0.5-based conditional flow-matching policy; exact action dimension and semantics are not reported',
    embodiments: ['Franka Research 3 with a Franka Hand whose fingers are replaced by bilateral DM-Tac WS sensors, two RealSense D435i cameras, and one wrist RealSense D405'],
    trainingDataSummary: 'TacAura supplies 100 human-teleoperated demonstrations per task across plug insertion, USB insertion, stamp pressing, and whiteboard erasing. A JEPA-style auxiliary branch uses current touch and subsequent actions to predict future visual-feature changes during training; that predictive branch is removed at inference, while tactile input remains part of the deployed policy.',
    realRobotEvaluation: 'Each fixed model variant is evaluated for 20 trials per task across four physical tasks. The best fixed variant by the paper’s reported mean is τ-Wrist at 71.25%, with full-task success of 60%, 40%, 90%, and 95%; a separate sentence mixes best task values from different variants and is not treated as one checkpoint result.',
    availability: 'The arXiv v3 paper and project page are public. The project labels code “Coming Soon,” but its button points to a separate STOLA placeholder; no τ implementation, model weights, TacAura download, data-conversion tools, or reusable artifact license was verified.',
    ...
    paperUrl: 'https://arxiv.org/abs/2607.24485',
    projectUrl: 'https://cocacola-lab.github.io/tau-Page/',
    tactileInput: 'yes',
    evidenceLimitations: 'τ is a July 2026 preprint with author-run results on one Franka Research 3 and bilateral DM-Tac WS configuration. Baselines marked with a dagger were adapted to the authors’ tactile setup; results do not establish cross-embodiment or cross-sensor transfer. The paper does not report exact action semantics or deployment rate, omits tactile-aware world-model comparisons for computational reasons, and had not released code, weights, or TacAura by 2026-08-22. “No deployment overhead” applies only to removing the training-only predictive branch, not to the tactile sensing and encoding path used at inference.',
    primarySources: [
      {
        label: 'τ preprint',
      ...
      {
        label: 'τ official project',
        url: 'https://cocacola-lab.github.io/tau-Page/',
        type: 'project',
      },
    ],
    sourceReviewed: '2026-08-22',
  },
  {
    id: 'unitacvla',
    name: 'UniTacVLA',
    organization: 'Harbin Institute of Technology, Great Bay University, Shanghai Jiao Tong University, Fudan University, Nanjing University, and Daimon Robotics',
    creatorOrganizations: ['Harbin Institute of Technology', 'Great Bay University', 'Shanghai Jiao Tong University', 'Fudan University', 'Nanjing University', 'Daimon Robotics'],
    releaseDate: '2026-06-30',
    category: 'VLA',
    inputModalities: ['Natural-language instruction', 'Wrist and first-person RGB observations', 'Robot state', 'Bilateral depth and shear tactile deformation', 'Predicted future tactile latents'],
    outputType: 'Low-frequency π0.5 action chunks plus bounded high-frequency residual corrections conditioned on current and predicted tactile latents; exact action dimensions and rates are not reported',
    embodiments: ['RealMan RM75B with a 3D-printed parallel gripper, bilateral DM-Tac W fingertip sensors, a wrist RealSense D405, and a first-person RealSense L515'],
    trainingDataSummary: 'The paper reports approximately one hour of 30 Hz demonstrations per subtask across eight subtasks, with 80% normal expert demonstrations and 20% disturbance-recovery demonstrations. It does not disclose trajectory or frame counts, a named public dataset, an exact split manifest, or a downloadable corpus.',
    realRobotEvaluation: 'Eight subtasks are evaluated in clean and perturbed settings with 50 trials per subtask-setting. The paper publishes all 16 task-setting success rates but not an aggregate number, so RoboSkin preserves the per-task protocol rather than presenting a calculated mean as a paper-reported score.',
    availability: 'The arXiv v1 paper is public, while the official repository contains only a “coming soon” project-page placeholder. No implementation, checkpoint, training dataset, release, tag, or reuse license was verified.',
  ...
  },
  {
    ...
    creatorOrganizations: ['National University of Singapore'],
    releaseDate: '2025-07-23',
    category: 'VLA',
    inputModalities: ['Natural-language task goal', 'Overhead and wrist RGB observations', 'Robot proprioception', 'GelSight Mini tactile images', 'Octopi tactile-language descriptions', 'Marker-tracked low-dimensional tactile state'],
    outputType: 'High-level primitive instruction plus an RDT-1B 64-step action chunk refined by a tactile and vision-conditioned interpolant controller',
    embodiments: ['Franka Emika Panda with a Robotiq 2F-140 gripper, one GelSight Mini on one gripper finger, and overhead plus wrist RealSense cameras'],
    trainingDataSummary: 'The base RDT-1B policy is first aligned to the Franka action space and then fine-tuned per task without tactile input. The interpolant controller is trained on paired VLA-generated and expert action sequences from 380 demonstrations across the Cup, Wipe, and Peel task pipelines; the exact RDT task-checkpoint release is not verified.',
    realRobotEvaluation: 'The complete planning-and-manipulation system is evaluated for 20 trials on each of three tasks and reports 9/20 complete Cup trials, 12/20 complete Wipe trials, and 7/20 complete Peel trials. Separate component tables also use 20 trials per condition and must not be merged with these end-to-end counts.',
    availability: 'The official MIT-licensed repository publishes partial controller and Octopi code and links a public processed dataset plus controller checkpoints. The modified RDT inference implementation is still marked for future release, no complete task-fine-tuned RDT checkpoint was verified, and the repository has no GitHub release.',
